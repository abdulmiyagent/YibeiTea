...
export function Footer() {
  ...
  // Fetch social links from store settings
  ...
  const subscribeMutation = api.newsletter.subscribe.useMutation({
    onSuccess: (data) => {
      setStatus("success");
      setMessage(t(`newsletter.messages.${data.message}`));
      setEmail("");
    },
    onError: (error) => {
      setStatus("error");
      if (error.message === "already_subscribed") {
        setMessage(t("newsletter.messages.already_subscribed"));
      } else {
        setMessage(t("newsletter.messages.error"));
      }
    },
  });
      ...
      <div className="container-custom section-padding">
        <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-4">
          ...
          <div className="space-y-4">
            ...
            {status === "success" ? (
              <div className="flex items-center gap-2 rounded-lg bg-tea-100 p-3 text-sm text-tea-800">
